...
function Filho(){
 ...
 <section className="rounded-2xl border border-[#eadcf9] bg-[#fbf6ff] p-4"><div className="flex items-center gap-4"><div className="grid size-20 shrink-0 place-items-center rounded-full bg-[#eee2ff] text-primary text-4xl">👶🏻</div><div><h2 className="text-lg font-bold">{name}</h2><p className="mt-1 text-[12px]">2 anos e 8 meses</p><p className="text-[12px] text-muted-foreground">{new Date(birth+"T00:00:00").toLocaleDateString("pt-BR")}</p></div></div></section>
...
function Info({icon,label,value}:{icon:React.ReactNode;label:string;value:string}){return <div className="flex items-center gap-3 py-3"><span className="text-primary">{icon}</span><span className="flex-1 text-[12px] font-medium">{label}</span><span className="text-[12px] font-semibold">{value}</span></div>}
function DateRow({label,value}:{label:string;value:string}){return <div className="flex items-center justify-between py-3 text-[12px]"><span>{label}</span><span className="font-semibold">{value}</span></div>}
