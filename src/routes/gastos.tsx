    ...
    <AppShell title="Meu Orçamento" subtitle="Entenda para onde seu dinheiro está indo e acompanhe sua evolução.">
      <section className="rounded-[28px] border border-[#e8def7] bg-gradient-to-br from-[#fbf8ff] to-white p-4 shadow-[0_12px_30px_rgba(75,45,120,.06)] sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[.1em] text-primary">Visão mensal</p>
            <h2 className="mt-1 text-[20px] font-bold capitalize">{fullMonthLabel(month)}</h2>
          </div>
          <span className="grid size-10 place-items-center rounded-2xl bg-white text-primary shadow-sm"><CalendarDays className="size-5" /></span>
        </div>
        <div className="mt-4 flex items-center justify-between rounded-2xl bg-white p-2 shadow-sm">
          <button onClick={() => setSelectedMonth(prev)} aria-label="Mês anterior" className="grid size-10 place-items-center rounded-xl hover:bg-muted"><ArrowLeft className="size-4" /></button>
          <div className="text-center"><p className="text-[9px] text-muted-foreground">Mês selecionado</p><p className="text-[13px] font-bold capitalize">{fullMonthLabel(month)}</p></div>
          <button onClick={() => setSelectedMonth(next)} disabled={next > now} aria-label="Próximo mês" className="grid size-10 place-items-center rounded-xl hover:bg-muted disabled:cursor-not-allowed disabled:opacity-30"><ArrowRight className="size-4" /></button>
        </div>
        <div className="mt-4 grid grid-cols-3 gap-2">
          <MiniStat label="Entradas" value={brl(entries)} tone="success" />
          <MiniStat label="Gastos" value={brl(spent)} tone="danger" />
          <MiniStat label="Resultado" value={brl(result)} tone={result >= 0 ? "primary" : "danger"} />
        </div>
      </section>

      <section className="mt-3 rounded-[24px] border border-border bg-white p-4">
        <div className="flex items-center justify-between"><div><h2 className="text-sm font-bold">Evolução dos últimos 6 meses</h2><p className="mt-0.5 text-[11px] text-muted-foreground">Veja a tendência das suas entradas e gastos.</p></div><BarChart3 className="size-5 text-primary" /></div>
        <div className="mt-5 flex h-40 items-end gap-2">
          {history.map((item) => <button key={item.key} onClick={() => setSelectedMonth(item.key)} className="group flex h-full min-w-0 flex-1 flex-col justify-end" title={`Ver ${fullMonthLabel(item.key)}`}>
            <div className="flex h-full items-end justify-center gap-1">
              <span className={`w-2.5 rounded-t-md transition-all ${item.key === month ? "bg-primary" : "bg-[#cdb9f4]"}`} style={{ height: `${Math.max(item.spent > 0 ? 8 : 2, (item.spent / maxHistory) * 100)}%` }} />
              <span className={`w-2.5 rounded-t-md transition-all ${item.key === month ? "bg-success" : "bg-[#bfe8d3]"}`} style={{ height: `${Math.max(item.entries > 0 ? 8 : 2, (item.entries / maxHistory) * 100)}%` }} />
            </div>
            <span className={`mt-2 text-[9px] font-semibold capitalize ${item.key === month ? "text-primary" : "text-muted-foreground"}`}>{item.label}</span>
          </button>)}
        </div>
        <div className="mt-3 flex justify-center gap-4 text-[9px] text-muted-foreground"><span className="flex items-center gap-1"><i className="size-2 rounded-full bg-primary" /> Gastos</span><span className="flex items-center gap-1"><i className="size-2 rounded-full bg-success" /> Entradas</span></div>
      </section>

      <section className="mt-3 rounded-2xl border border-border bg-white p-4">
        <h2 className="text-sm font-bold">Comparação com {fullMonthLabel(prev)}</h2>
        <div className="mt-3 grid grid-cols-2 gap-3"><div className="rounded-xl bg-muted p-3"><p className="text-[11px] text-muted-foreground">{monthLabel(prev)}</p><p className="mt-1 text-base font-bold">{brl(spentPrev)}</p></div><div className="rounded-xl bg-muted p-3"><p className="text-[11px] text-muted-foreground">{monthLabel(month)}</p><p className="mt-1 text-base font-bold">{brl(spent)}</p></div></div>
        <p className={`mt-2 text-[11px] font-semibold ${diff > 0 ? "text-destructive" : "text-success"}`}>{spentPrev === 0 ? "Sem gastos no mês anterior para comparar." : diff === 0 ? "Você gastou o mesmo do mês passado." : `${diff > 0 ? "Você gastou" : "Você economizou"} ${brl(Math.abs(diff))}${diffPercent !== null ? ` (${Math.abs(diffPercent)}%)` : ""} ${diff > 0 ? "a mais" : "a menos"} que no mês passado.`}</p>
      </section>

      <section className="mt-3 rounded-2xl border border-border bg-white p-4">
        <h2 className="text-sm font-bold">Gastos por categoria</h2>
        {cats.length === 0 ? <p className="mt-2 text-[12px] text-muted-foreground">Nenhum gasto registrado neste mês ainda.</p> : <div className="mt-4 flex items-center gap-5"><div className="relative grid size-28 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(${cats.map((c, i) => `${colors[i]} ${(cats.slice(0, i).reduce((a, x) => a + x.total, 0) / Math.max(spent, 1)) * 360}deg ${(cats.slice(0, i + 1).reduce((a, x) => a + x.total, 0) / Math.max(spent, 1)) * 360}deg`).join(",")})` }}><div className="grid size-16 place-items-center rounded-full bg-white"><span className="text-[10px] text-muted-foreground">total<br /><b className="text-[11px] text-foreground">{brl(spent)}</b></span></div></div><div className="min-w-0 flex-1 space-y-2">{cats.map((c, i) => <div key={c.category} className="flex items-center gap-2 text-[11px]"><span className="size-2 shrink-0 rounded-full" style={{ background: colors[i] }} /><span className="flex-1 truncate">{c.category}</span><b>{Math.round((c.total / Math.max(spent, 1)) * 100)}%</b><span>{brl(c.total)}</span></div>)}</div></div>}
      </section>

      <section className="mt-3 rounded-2xl border border-[#eadcf9] bg-[#fbf6ff] p-4">
        <div className="flex items-center gap-2"><span className="grid size-8 place-items-center rounded-xl bg-[#eee4ff] text-primary"><PiggyBank className="size-4" /></span><div><h2 className="text-sm font-bold">Gastos com meu filho</h2><p className="text-[11px] text-muted-foreground">Somente lançamentos marcados como do filho.</p></div></div>
        <p className="mt-3 text-2xl font-bold text-primary">{brl(childSpent)}</p><div className="mt-2 h-2 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-primary" style={{ width: `${spent > 0 ? Math.min(100, (childSpent / spent) * 100) : 0}%` }} /></div><p className="mt-1 text-[10px] text-muted-foreground">{spent > 0 ? `${Math.round((childSpent / spent) * 100)}% do total de gastos` : "Registre gastos para ver esta proporção"}</p>
      </section>

      <section className="mt-3 rounded-2xl border border-border bg-white p-4">
        <div className="flex gap-2">{(["gastos", "entradas"] as const).map((t) => <button key={t} onClick={() => setTab(t)} className={`rounded-full px-3 py-1.5 text-[11px] font-bold ${tab === t ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}>{t === "gastos" ? "Gastos do mês" : "Entradas do mês"}</button>)}</div>
        {tab === "gastos" ? <div className="mt-3 divide-y divide-border">{monthExpenses.length === 0 && <p className="py-3 text-[12px] text-muted-foreground">Nenhum gasto registrado neste mês.</p>}{monthExpenses.map((e) => <div key={e.id} className="flex items-center gap-2 py-2.5 text-[12px]"><div className="min-w-0 flex-1"><p className="truncate font-semibold">{e.description} {e.forChild && <Baby className="inline size-3 text-primary" />}</p><p className="text-muted-foreground">{dayLabel(e.date)} · {e.category}</p></div><b className="text-destructive">{brl(e.amount)}</b><button aria-label="Editar gasto" onClick={() => setEditing(e)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted"><Pencil className="size-4" /></button><button aria-label="Excluir gasto" onClick={() => { removeExpense(e.id); toast.success("Gasto excluído."); }} className="rounded-lg p-1.5 text-destructive hover:bg-muted"><Trash2 className="size-4" /></button></div>)}</div> : <div className="mt-3 divide-y divide-border">{monthIncomes.length === 0 && <p className="py-3 text-[12px] text-muted-foreground">Nenhuma entrada registrada neste mês.</p>}{monthIncomes.map((i) => <div key={i.id} className="flex items-center gap-2 py-2.5 text-[12px]"><div className="min-w-0 flex-1"><p className="truncate font-semibold">{i.description}</p><p className="text-muted-foreground">{dayLabel(i.date)} · {i.source}</p></div><b className="text-success">{brl(i.amount)}</b><button aria-label="Excluir entrada" onClick={() => { removeIncome(i.id); toast.success("Entrada excluída."); }} className="rounded-lg p-1.5 text-destructive hover:bg-muted"><Trash2 className="size-4" /></button></div>)}</div>}
        <div className="mt-4 flex gap-2"><Button className="flex-1" onClick={() => setExpenseOpen(true)}><Plus className="size-4" /> Gasto</Button><Button variant="secondary" className="flex-1" onClick={() => setIncomeOpen(true)}><Plus className="size-4" /> Entrada</Button></div>
      </section>
...
function MiniStat({ label, value, tone }: { label: string; value: string; tone: "success" | "danger" | "primary" }) {
  ...
  return <div className="rounded-2xl bg-white p-3 shadow-sm"><p className="text-[9px] text-muted-foreground">{label}</p><p className={`mt-1 truncate text-[12px] font-bold ${c}`}>{value}</p></div>;
}
...
function ExpenseForm({ initial, onSubmit, onClose }: { initial?: Expense; onSubmit: (values: { amount: number; date: string; description: string; category: ExpenseCategory; forChild: boolean }) => void; onClose: () => void }) {
  const [amount, setAmount] = useState(initial ? String(initial.amount) : ""); const [description, setDescription] = useState(initial?.description ?? ""); const [category, setCategory] = useState<ExpenseCategory>(initial?.category ?? "Outros"); const [date, setDate] = useState(initial?.date ?? today()); const [forChild, setForChild] = useState(initial?.forChild ?? false);
  function submit(e: FormEvent) { e.preventDefault(); const value = Number(amount); if (!value || !description.trim()) { toast.error("Informe valor e descrição."); return; } onSubmit({ amount: value, date, description: description.trim(), category, forChild }); onClose(); }
...
function IncomeForm({ onSubmit, onClose }: { onSubmit: (values: { amount: number; date: string; description: string; source: IncomeSource }) => void; onClose: () => void }) {
  const [amount, setAmount] = useState(""); const [description, setDescription] = useState(""); const [source, setSource] = useState<IncomeSource>("Salário"); const [date, setDate] = useState(today());
  function submit(e: FormEvent) { e.preventDefault(); const value = Number(amount); if (!value || !description.trim()) { toast.error("Informe valor e descrição."); return; } onSubmit({ amount: value, date, description: description.trim(), source }); onClose(); }
